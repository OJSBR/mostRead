 ...
 * plugins/blocks/mostRead, the OJS 3.5 installation around it is bootstrapped so the
...
if (!class_exists('\PKP\plugins\GenericPlugin')) {
    ...
    if (!is_file($root . '/lib/pkp/includes/bootstrap.php')) {
        fwrite(STDERR, "The plugin must be installed in plugins/blocks/mostRead of an OJS 3.5 installation to run the suite.\n");
        exit(2);
    }
    chdir($root);
